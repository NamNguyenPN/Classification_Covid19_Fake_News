...
class contractions:
    ...
    def fix(self,sentence):
        if (sentence in self.contractions):
            return self.contractions[sentence]
        return sentence
...
def remove_SP(sentence):
    return re.sub(r'[^a-zA-Z\d\s\']+', '', sentence)
...
def remove_stopword(sentence):
    ...
    return " ".join([each_word for each_word in sentence if each_word not in stopwords.words('english')])

def normalize(sentence):
    sentence=remove_SP(sentence)
    sentence=remove_SP(sentence)
